...
export default function SerialRegistryPage() {
   ...
   const headerActions = (
      <>
         ...
         <div className="flex flex-wrap gap-4 w-full sm:w-auto justify-end">
            <Button variant="secondary" className="flex-1 sm:flex-none px-6 h-10" onClick={handleExport}>
               <Download className="w-4 h-4 text-primary mr-2" /> Export Dataset
            </Button>
            <Button className="flex-1 sm:flex-none px-6 h-10 italic" onClick={() => setIsQuickEntryOpen(true)}>
   ...
   return (
      <div className="space-y-8 pb-12">
         ...
         {/* Filter Bar */}
         <div className="bg-white p-4 rounded-3xl border border-border-main shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4">
                     ...
                     {serialNumberRegistry.map(item => (
                        <tr key={item.id} className="group hover:bg-gray-50/50 transition-all cursor-pointer" onClick={() => handleActionClick(item.asset)}>
                           ...
                           <td className="px-6 py-6">
                              <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-tighter ${item.status === 'OPERATIONAL' ? 'bg-blue-50 text-primary border border-blue-100' : 'bg-orange-50 text-orange-600 border border-orange-100'
                                 }`}>
                                 {item.status}
                              </span>
                           </td>
               ...
               <div className="p-8 space-y-8 relative before:absolute before:left-[19px] before:top-10 before:bottom-10 before:w-[2px] before:bg-gray-100">
                  {auditLogs.map(log => (
                     <div key={log.id} className="flex gap-6 relative z-10 group cursor-pointer" onClick={() => toast.info(`Viewing details for: ${log.title}`)}>
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white shadow-xl ${log.type === 'System' ? 'bg-primary' : log.type === 'User' ? 'bg-gray-400' : 'bg-warning'
