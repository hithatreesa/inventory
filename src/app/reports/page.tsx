      ...
      <div className="flex flex-col lg:flex-row justify-between lg:items-center bg-white p-8 rounded-3xl border border-border-main shadow-sm gap-6">
        <div>
           <div className="flex items-center gap-3 mb-1">
              ...
              <Badge variant="secondary" className="h-6 px-3 rounded-full border-primary/20 bg-primary/5 text-primary text-[9px] font-black italic">ANALYTICS_V3</Badge>
           </div>
           <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-1 italic">Enterprise Intelligence & Data aggregation Center</p>
        </div>
         ...
         <div className="overflow-x-auto min-h-[400px]">
            <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-gray-50">
                  {[1, 2, 3, 4, 5].map((row) => (
                    <tr key={row} className="group hover:bg-gray-50/20 transition-all cursor-pointer">
                       <td className="px-6 py-6 font-bold text-text-main italic">
                          <p className="text-sm font-black text-[#003366] uppercase leading-none">{activeTab === 'inventory' ? 'X-5 Pro Processor' : 'Mar ' + (row + 10) + ', 2024'}</p>
                          <p className="text-[9px] font-black text-gray-300 uppercase mt-1 tracking-widest">Digital Stamp: {Math.random().toString(36).substr(2, 6).toUpperCase()}</p>
                       </td>
                       <td className="px-6 py-6">
                          <code className="text-[10px] bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100 font-mono font-black text-gray-400">{activeTab === 'purchase' ? 'PO-992' + row : 'INV-' + row + '092'}</code>
                       </td>
                       <td className="px-6 py-6">
                       ...
                       <td className="px-6 py-6 text-right font-black italic text-sm text-[#003366] tabular-nums tracking-tighter">
                          ₹{(Math.random() * 12000 + 1000).toLocaleString(undefined, {minimumFractionDigits: 2})}
                       </td>
                       <td className="px-8 py-6 text-right">
                          <Badge variant={row % 2 ===0 ? 'success' : 'neutral'} className="h-6 px-3 rounded-lg font-black italic text-[9px] uppercase tracking-tighter">{row % 2 === 0 ? 'Verified' : 'Pending'}</Badge>
                       </td>
                    </tr>
                  ))}
               </tbody>
